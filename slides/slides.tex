...
\begin{block} {Oxford Buildings Dataset}
	...
	\end{itemize}
\end{block}
	Random sampled subsets to improve compression and benchmark speed!
\end{frame}
...
VlBenchmark uses a slightly different scheme, but the calculation stays the same.
	\begin{block}{How use the four query classes}
		\begin{itemize}
			...
			\item bad will count as irrelevant
		\end{itemize}
	\end{block}
...
\begin{frame} {Retrieval System}
	\begin{block}{Ranking}
		\begin{itemize}
			\item calculate reference descriptors
...
\begin{frame}{Difference of Gaussian}
  \includegraphics[width=0.9\textwidth]{img/DoG.png}
\end{frame}
...
\begin{frame}{Accurate keypoint localization}
  \begin{columns}
    \begin{column}{.5\textwidth}
      ...
      \item Large number of extrema, computationally expensive
      \end{itemize}
    \end{column}
    \begin{column}{.5\textwidth}
      \includegraphics[width=0.8\textwidth]{img/key_localization.png}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}
  \center\Huge{\( feature_{i} = x_{i}, y_{i}, s_{i} \)}
\end{frame}
...
\begin{frame}
	\begin{tabular}{|l|l|l|}
		& SIFT & SURF\\
	Key point detection & Local extrema De  
	\end{tabular}
\end{frame}
\subsection{PHOW}
\begin{frame}{PHOW}
	The Phow features are a variation of dense SIFT descriptors, extracted at multiple scales. A color version, extracts descriptors on the three(rgb,hsv,...) channels and stacks them up.
\end{frame}
