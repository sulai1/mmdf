...
% AUTORIGHTS
  ...
  methods
    function obj = VggRetrievalDataset(varargin)
      ...
      %%check if it is a custom dataset
      if(ismember(obj.Opts.category,obj.AllCategories))
    ...
    function imgPath = getImagePath(obj,imageNo)
      ...
      if ~isempty(imageNo) && imageNo >= 1 && imageNo <= obj.NumImages
          x = obj.Images.paths{imageNo};
        imgPath = fullfile(obj.ImagesDir,x);
      else
  ...
  methods(Access = protected)
    ...
    function [images queries] = buildImageDatabase(obj)
      ...
      for i = 1:numel(names)
        ...
        if isoxbuild || obj.IsCustom, imageName = imageName(6:end); end;
        queries(i).name = name ;
